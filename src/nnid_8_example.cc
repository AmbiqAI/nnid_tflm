...
main(void) {
    ...
    int status = nnid_8_minimal_init(&model); // model init with minimal defaults

    if (status == nnid_8_STATUS_FAILURE) {
        while (1)
            example_status = nnid_8_STATUS_INIT_FAILED; // hang
    }
    ...
    for (int i = 0; i < numInputs; i++) {
        memcpy(model.model_input[i]->data.int8, ((char *)nnid_8_example_input_tensors) + offset,
    ...
    if (invoke_status != kTfLiteOk) {
        while (1)
            example_status = nnid_8_STATUS_FAILURE; // invoke failed, so hang
    }
    ...
    for (int i = 0; i < numOutputs; i++) {
        if (0 != memcmp(model.model_output[i]->data.int8,
                        ((char *)nnid_8_example_output_tensors) + offset,
                        model.model_output[i]->bytes)) {
            while (1)
                example_status = nnid_8_STATUS_INVALID_CONFIG; // miscompare, so hang
        }
    ...
    while (1) {
        // Success!
        example_status = nnid_8_STATUS_SUCCESS;
    }
}
